...
//			}
		...
		if (!ent->groundentity || (ent->flags & FL_FLY))
		{
			...
			if (	!(ent->flags & FL_FLY)
				...
				&& (land_node = level.node_data->nodes[ent->nav_data.goal_index - 1]) != 0) // HYPOV8_ADD !=0
//				&&	(land_node->node_type & NODE_LANDING))
			...
			if (	(ent->flags & FL_FLY)
				&& ((land_node = level.node_data->nodes[ent->nav_data.goal_index - 1]) != 0 || ((ent->flags &= ~FL_FLY) && false)) // HYPOV8_ADD !=0
				/*&&	(land_node->node_type & NODE_LANDING)*/)
