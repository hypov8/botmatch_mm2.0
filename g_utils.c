...
//
	...
	{
		t = NULL;
		while ((t = G_Find(t, FOFS(targetname), ent->killtarget)) != 0)// HYPOV8_ADD
		{
			G_FreeEdict (t);
			if (!ent->inuse)
			{
		...
		t = NULL;
		while ((t = G_Find(t, FOFS(target2), ent->killtarget)) != 0)// HYPOV8_ADD
		{
			G_FreeEdict (t);
			if (!ent->inuse)
			{
...
//
	...
	{
		t = NULL;
		while ((t = G_Find(t, FOFS(targetname), ent->target)) != 0)// HYPOV8_ADD
		{
...
void G_FreeEdict (edict_t *ed)
{
	...
	// HYPOV8_ADD allready unlinked!!! kpded2 caught error from  droptofloor()
