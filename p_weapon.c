...
// HYPOV8_END
	...
	//hypov8. invalid if they dont have the spistol allready!!!
	//if (other->client->pers.weapon == FindItem ("SPistol"))
			//other->client->pers.silencer_shots = 20;
	
	//hypov8 add. spistol fix.
	if (!(strcmp(ent->classname, "weapon_spistol")))
		other->client->pers.silencer_shots = 20;
...
// HYPOV8_ADD everything ok, if the user selected pistol manualy. store it in client
	if (strcmp(item->pickup_name, "Pistol") == 0 || strcmp(item->pickup_name, "SPistol") == 0) //hypov8 todo:add crowbar
...
#define FRAME_IDLE_FIRST		(FRAME_FIRE_LAST + 1) // HYPOV8 FRAME_RELOAD_LAST. (FRAME_FIRE_LAST + 1)
...
// RAFAEL 01-11-99
	...
	if (ent->client->weaponstate == WEAPON_RELOAD_CYCLE)
	{
		...
		*/
		{
			ent->client->ps.gunframe += 2; // HYPOV8_ADD //FREDZ fast shotgun reload
			//ent->client->ps.gunframe++;
			return;
		}
	}
	...
	if (ent->client->weaponstate == WEAPON_DROPPING)
	{
		...
		{
			if (ent->client->pers.silencer_shots <= 0 && ent->client->ps.gunframe == FRAME_OFFSET_FOR_SPISTOL_WITH_SILENCER) //hypov8 was 0
			{
				ChangeWeapon (ent);
				return;
			}
...
void Weapon_Blackjack (edict_t *ent)
{
	...
	//HITMEN auto swutch bug. remove prev gun no lerp
...
void Weapon_Pistol (edict_t *ent)
{
	...
	if (ent->client->ps.gunframe == 1) //hypov8 at first frame??
...
void Weapon_SPistol_Fire (edict_t *ent)
{
	...
	if (sv_hitmen->value /*enable_hitmen*/)
	{
		ent->client->pers.silencer_shots = 50;
		damage = 50;
	}
	// END
	...
	if (ent->client->ps.gunframe == 21)
	{
		...
		{//hypov8 add. change wep after silencer expires
			ent->client->pers.inventory[ITEM_INDEX(ent->client->pers.weapon)] = 0;
			NoAmmoWeaponChange(ent);
...
// gi.dprintf ("silencer shots left: %d\n", ent->client->pers.silencer_shots);
	...
	{
		//if (sv_hitmen->value /*enable_hitmen*/ && ent->client->ps.gunframe == 8)
	...
	if (ent->client->ps.gunframe == 49 && !sv_hitmen->value /*enable_hitmen*/) // BEGIN HITMEN
	{
	...
	if (sv_hitmen->value /*enable_hitmen*/)
	{
		if (ent->client->ps.gunframe == 2)
			gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/holster.wav"), 1, ATTN_NORM, 0);
		if (ent->client->ps.gunframe == 26)
			gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/clip_out.wav"), 1, ATTN_NORM, 0);
		if (ent->client->ps.gunframe == 35)
			gi.sound(ent, CHAN_AUTO, gi.soundindex("weapons/pistol/clip_in.wav"), 1, ATTN_NORM, 0);
	}
	else
	if (ent->client->ps.gunframe == 1) //hypov8 1?
...
// ACEBOT_ADD
	//if (ent->acebot.is_bot)
		//ACEMV_Attack_CalcRandDir(ent, forward);
// ACEBOT_END
