...
void DropHook (edict_t *ent) {
  ...
  // acebot
  if (ent->owner->acebot.pm_hookActive)
	ent->owner->acebot.pm_last_node = INVALID;

  ent->owner->acebot.hookDistLast = 0;
  ent->owner->acebot.hookDistCurrent= 0;
  ent->owner->acebot.pm_hookActive = 0;
  //end
...
void MaintainLinks (edict_t *ent) {
  ...
  VectorCopy(ent->owner->s.origin, viewHeight);
  viewHeight[2] += 8;// ent->owner->viewheight;
...
#elseif 0
  gi.WriteByte (svc_temp_entity);
  gi.WriteByte (TE_GRAPPLE_CABLE);  //hypov8
  gi.WriteShort (ent - g_edicts);
  ...
  gi.WritePosition (vec3_origin);
	...
	gi.WritePosition (viewHeight);
	gi.WritePosition (ent->s.origin);
	gi.WriteDir(norm_hookvel);//tr.plane.normal);
...
void HookBehavior(edict_t *ent) {
  ...
  //add hypov8. entity count??
  //NAV_DrawLine(ent->s.origin, ent->owner->s.origin);
  ...
  //float NAV_Debug_DrawPath(edict_t *src, edict_t *dest);
	...
	// have we been swinging for long enough?
  ...
  // grow the length of the chain
	...
	// hypov8. set last length for bot
	ent->owner->acebot.hookDistLast = ent->owner->acebot.hookDistCurrent;
	if (ent->owner->acebot.is_bot)
	{
		if ((ent->owner->client->hookstate & SHRINK_ON) && (ent->angle > 2.0))
		{
			ent->angle -= hook_pull_speed->value;
			if (ent->angle < 2.0) ent->angle = 2.0;
			ent->owner->acebot.hookDistCurrent = ent->angle;
	...
	if ((ent->owner->client->hookstate & SHRINK_ON)	&& (ent->angle > hook_min_length->value)) 
	{
		ent->angle -= hook_pull_speed->value;
	...
	if(ent->enemy->client) 
	{
		...
		// vampire hook stuff
		if(((level.time - ent->owner->client->hook_vampire_time) >= 1.0) && hook_vampirism->value) 
		{
		  ent->owner->client->hook_vampire_time = level.time;

			if(hook_vampirism->value > 0) 
			{
				ent->owner->health += (int)(hook_vampirism->value * hook_vampire_ratio->value);
			...
			else //minus health //hypov8
			{
				...
				if ((ent->owner->health + hook_vampirism->value) <= 0)
				{
					...
					ent->owner->health += hook_vampirism->value;
			}
		}
	} 
	else 
...
void HookTouch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf) 
{
	...
	if ((other->solid == SOLID_BBOX)) 
	{
		if ((other->svflags & SVF_MONSTER) || (other->client)) 
		{
			...
			{
				safe_centerprintf(other, "%s has hooked you!", ent->owner->client->pers.netname);
				safe_centerprintf(ent->owner, "You've hooked %s!",	other->client->pers.netname);
			}
		}
...
//hook is traveling to a surface/player
void HookAirborne (edict_t *ent) {
	...
	if ( !(ent->owner->client->hookstate & HOOK_ON) || (chainlen > hook_max_length->value) ) 
	{
		DropHook(ent);
		return;
	}
	...
	if(hook_is_homing->value && hook_players->value) 
	{
		...
		while ((blip = findradius(blip, ent->s.origin, hook_homing_radius->value)) != NULL) 
		{
